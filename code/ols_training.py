"""
Subgoal 5: OLS Model & Data Visualization
Flower Classification using MNLogit (OLS) with PCA
...
try:
    ...
    raise FileNotFoundError("CSV files not found. Check paths!")
...
for class_id, class_name in FLOWER_CLASSES.items():
    print(f"  {class_id} ({class_name}): {(y_train==class_id).sum()} samples")
...
plt.figure()
plt.bar(['Train', 'Test'], [train_accuracy*100, test_accuracy*100], color=['#4ECDC4', '#FF6B6B'])
plt.ylabel('Accuracy (%)')
plt.ylim([0, 100])
...
# Per-class accuracy
class_acc = [(y_test_pred[y_test==i]==i).sum() / max((y_test==i).sum(),1)*100 for i in range(len(FLOWER_CLASSES))]
plt.figure()
plt.bar(class_names, class_acc, color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#98D8C8'])
plt.ylabel('Accuracy (%)')
plt.title('Per-Class Accuracy')
plt.ylim([0, 100])
plt.tight_layout()
plt.savefig(os.path.join(VIZ_DIR, 'accuracy_breakdown.png'))
plt.close()
...
# Comparison with ANN/CNN (replace with your ANN accuracy)
ann_accuracy = 47.36
report_content += f"\nComparison with ANN Model:\n"
report_content += f"ANN Test Accuracy: {ann_accuracy}%\n"
report_content += f"OLS Test Accuracy: {test_accuracy*100:.2f}%\n"
report_content += f"Difference: {test_accuracy*100 - ann_accuracy:+.2f}%\n"

with open(REPORT_FILE, 'w') as f:
    f.write(report_content)
print(f"Report saved: {REPORT_FILE}")

print("\n✅ OLS MODEL TRAINING & DOCUMENTATION COMPLETED SUCCESSFULLY!")
